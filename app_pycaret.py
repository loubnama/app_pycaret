...
def drop_columns(df, columns_to_drop):
    df = df.drop(columns=columns_to_drop)import pandas as pd
...
handle_missing_values_option = st.radio("كيف ترغب في التعامل مع القيم المفقودة؟", ["حذف الصفوف ذات القيم المفقودة", "ملء القيم المفقودة بقيمة محددة"])
...
if handle_missing_values_option == "حذف الصفوف ذات القيم المفقودة":
    ...
    for col in df.columns:
        if pd.api.types.is_numeric_dtype(df[col]):
            ...
            filling_value = df[col].mode()[0]  # احصل على القيمة الأكثر تكرارًا

        df[col].fillna(filling_value, inplace=True)

    st.write(f"تم ملء القيم المفقودة بقيمة: {filling_value}")
    st.write(df)


##
encode_categorical_option = st.radio("كيف ترغب في ترميز البيانات الفئوية؟", ["ترميز الفئات (One-Hot Encoding)", "ترميز التسلسلات (Label Encoding)"])
categorical_columns = df.select_dtypes(include=['object']).columns

if encode_categorical_option == "ترميز الفئات (One-Hot Encoding)":
    #قم بترميز الفئات باستخدام One-Hot Encoding
    df_encoded = pd.get_dummies(df, columns=categorical_columns)
    st.write("تم ترميز البيانات الفئوية باستخدام One-Hot Encoding:")
    st.write(df_encoded)
elif encode_categorical_option == "ترميز التسلسلات (Label Encoding)":
    # قم بترميز التسلسلات باستخدام Label Encoding
    label_encoder = LabelEncoder()
    for col in categorical_columns:
        df[col] = label_encoder.fit_transform(df[col])
    st.write("تم ترميز البيانات الفئوية باستخدام Label Encoding:")
    st.write(df)
...
if is_classification:
    st.write("المهمة هي تصنيف (Classification)")
    # اختيار X و Y
    X_options = df.drop(columns=[col]).columns
    Y_options = df[col].name
else:
    st.write("المهمة هي تحديد القيمة (Regression)")
    # اختيار X و Y
    X_options = df.drop(columns=[col]).columns
    Y_options = df[col].name


X = st.selectbox("اختر المتغير المستقل (X)", options=X_options)
if is_classification:
    Y = st.selectbox("اختر المتغير المستجيب (Y)", options=[Y_options])
else:
    # اختيار Y (العمود المستجيب) بناءً على نوع المهمة
    if is_classification:
        Y = st.selectbox("اختر المتغير المستجيب (Y)", options=[Y_options])
    else:
        Y = Y_options
...
# التحليل الإحصائي والرسم البياني لـ Y
if is_classification:
    # Classification analysis
    st.write("التحليل الإحصائي والرسم البياني للتصنيف (Classification):")
    ...
    # Regression analysis
    st.write("التحليل الإحصائي والرسم البياني للتحديد (Regression):")
